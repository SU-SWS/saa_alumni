...
const PerkCardView = (props) => {
  ...
  const perkTypeLink = perkTypeLinks[type] ?? perkTypeLinks["perk"];

  // Perk type taxonomy tag is just text if no link object is provided in dataSource.js
  let perkTypeTag =
  ...
  // If link object is provided for the perk type, use the SbLink component instead
  if (perkTypeLink !== "") {
    perkTypeTag =
      <SbLink
        link={perkTypeLink}
